...
void InitOption(void)
{
	...
	// オプションウィンドウの初期化
	g_Option[OPTION_WINDOW].vScl = D3DXVECTOR3(Screen.fWidth / 16 * 6,Screen.fHeight / 9 * 6,0.0f);
	g_Option[OPTION_WINDOW].vPos = D3DXVECTOR3(Screen.fWidth / 2,Screen.fHeight / 2,0.0f);
	// BGMミキサーバーの初期化
	...
	// 実行時の位置・スケールの設定
	for(int nOption = 0; nOption < MAX_OPTION; nOption++)
	{
		g_Option[nOption].vPosOld = g_Option[nOption].vPos;
		g_Option[nOption].vPosInStartClient = g_Option[nOption].vPos;
		g_Option[nOption].vSclInStartClient = g_Option[nOption].vScl;
	}
	// オプションウィンドウの頂点座標の設定
...
void UpdateOption(void)
{
	...
	if(g_bOptionWindow == true)
	{
		...
		if(g_Option[BGM_MIXERBAR_ICON].bChoice == true)
		{
			...
			g_Option[BGM_MIXERBAR_ICON].vPosInStartClient.x += (g_Option[BGM_MIXERBAR_ICON].vPos.x - g_Option[BGM_MIXERBAR_ICON].vPosOld.x)
																/ Screen.fRatioNewWidthToOldWidth;
			// 頂点情報の変更
		...
		if(g_Option[SE_MIXERBAR_ICON].bChoice == true)
		{
			...
			// 実行時のクライアント領域での位置を変更
			g_Option[SE_MIXERBAR_ICON].vPosInStartClient.x += (g_Option[SE_MIXERBAR_ICON].vPos.x - g_Option[SE_MIXERBAR_ICON].vPosOld.x)
																/ Screen.fRatioNewWidthToOldWidth;
			// 頂点情報の変更
...
void SetVertexUI2D(UI_2D *pUI, int nMaxNum)
{
	for(int nNumUI = 0 ; nNumUI < nMaxNum ; nNumUI++, pUI++)
	{
		{// 頂点バッファの中身を埋める
			...
			pUI -> pD3DVtxBuff -> Lock(0,0,(void**)&pVtx,0);
			// 頂点座標の設定
			pVtx[0].vtx = D3DXVECTOR3(pUI -> vPosInStartClient.x - pUI -> vSclInStartClient.x / 2, pUI -> vPosInStartClient.y - pUI -> vSclInStartClient.y / 2, 0.0f);
			pVtx[1].vtx = D3DXVECTOR3(pUI -> vPosInStartClient.x + pUI -> vSclInStartClient.x / 2, pUI -> vPosInStartClient.y - pUI -> vSclInStartClient.y / 2, 0.0f);
			pVtx[2].vtx = D3DXVECTOR3(pUI -> vPosInStartClient.x - pUI -> vSclInStartClient.x / 2, pUI -> vPosInStartClient.y + pUI -> vSclInStartClient.y / 2, 0.0f);
			pVtx[3].vtx = D3DXVECTOR3(pUI -> vPosInStartClient.x + pUI -> vSclInStartClient.x / 2, pUI -> vPosInStartClient.y + pUI -> vSclInStartClient.y / 2, 0.0f);
			// rhwの設定
			pVtx[0].rhw = 1.0f;
			pVtx[1].rhw = 1.0f;
			pVtx[2].rhw = 1.0f;
...
void RecalculationOption(void)
{
	...
	// オプションウィンドウの再計算
	g_Option[OPTION_WINDOW].vScl = D3DXVECTOR3(Screen.fWidth / 16 * 6,Screen.fHeight / 9 * 6,0.0f);
	g_Option[OPTION_WINDOW].vPos = D3DXVECTOR3(Screen.fWidth / 2.0f,Screen.fHeight / 2,0.0f);
	// BGMミキサーバーの再計算
	g_Option[BGM_MIXERBAR].vScl = D3DXVECTOR3(g_Option[OPTION_WINDOW].vScl.x,g_Option[OPTION_WINDOW].vPos.y / 3,0.0f);
	g_Option[BGM_MIXERBAR].vPos = D3DXVECTOR3(g_Option[OPTION_WINDOW].vPos.x,g_Option[OPTION_WINDOW].vPos.y - (g_Option[OPTION_WINDOW].vScl.y / 4),0.0f);
	// SEミキサーバーの再計算
	...
	// BGMミキサーバーのアイコンの再計算
	g_Option[BGM_MIXERBAR_ICON].vScl = D3DXVECTOR3(g_Option[OPTION_WINDOW].vScl.x / 10,g_Option[OPTION_WINDOW].vScl.y / 10,0.0f);
	g_Option[BGM_MIXERBAR_ICON].vPos = D3DXVECTOR3(g_Option[BGM_MIXERBAR_ICON].vPosInStartClient.x * Screen.fRatioNewWidthToOldWidth,
													g_Option[BGM_MIXERBAR_ICON].vPosInStartClient.y * Screen.fRatioNewHeightToOldHeight,
													0.0f);
	// SEミキサーバーのアイコンの再計算
	g_Option[SE_MIXERBAR_ICON].vScl = D3DXVECTOR3(g_Option[OPTION_WINDOW].vScl.x / 10,g_Option[OPTION_WINDOW].vScl.y / 10,0.0f);
	g_Option[SE_MIXERBAR_ICON].vPos = D3DXVECTOR3(g_Option[SE_MIXERBAR_ICON].vPosInStartClient.x * Screen.fRatioNewWidthToOldWidth,
													g_Option[SE_MIXERBAR_ICON].vPosInStartClient.y * Screen.fRatioNewHeightToOldHeight,
													0.0f);
#else
	for(int nOption = 0; nOption < MAX_OPTION; nOption++)
	{
		// 現在のクライアント領域でのスケールを計算
		g_Option[nOption].vScl = D3DXVECTOR3
		(
			g_Option[nOption].vSclInStartClient.x * Screen.fRatioNewWidthToOldWidth,	// 実行時のサイズ × 現在の横幅比
			g_Option[nOption].vSclInStartClient.y * Screen.fRatioNewHeightToOldHeight,	// 実行時のサイズ × 現在の縦幅比
			0.0f
		);
		// 現在のクライアント領域での位置を計算
		g_Option[nOption].vPos = D3DXVECTOR3
		(
			g_Option[nOption].vPosInStartClient.x * Screen.fRatioNewWidthToOldWidth,	// 実行時の位置 × 現在の横幅比
			g_Option[nOption].vPosInStartClient.y * Screen.fRatioNewHeightToOldHeight,	// 実行時の位置 × 現在の縦幅比
			0.0f
		);
	}
#endif
