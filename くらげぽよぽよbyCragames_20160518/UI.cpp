...
void UpdateOption(void)
{
	...
	if(g_bOptionWindow == true)
	{
		...
		if(g_Option[BGM_MIXERBAR_ICON].bChoice == true)
		{
			...
			g_Option[BGM_MIXERBAR_ICON].vPosInStartClient.x += (g_Option[BGM_MIXERBAR_ICON].vPos.x - g_Option[BGM_MIXERBAR_ICON].vPosOld.x) / Screen.fRatioNewWidthToOldWidth;
			// 頂点情報の変更
		...
		if(g_Option[SE_MIXERBAR_ICON].bChoice == true)
		{
			...
			// 実行時のクライアント領域での位置を変更
			g_Option[SE_MIXERBAR_ICON].vPosInStartClient.x += (g_Option[SE_MIXERBAR_ICON].vPos.x - g_Option[SE_MIXERBAR_ICON].vPosOld.x) / Screen.fRatioNewWidthToOldWidth;
			// 頂点情報の変更
