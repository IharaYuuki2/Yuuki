...
typedef struct UI_2D
{
	...
	D3DXVECTOR3 vPos;								// 現在のクライアント領域の座標
	D3DXVECTOR3 vScl;								// 現在のクライアント領域でのサイズ
	D3DXVECTOR3 vPosOld;							// １フレーム前のクライアント領域の座標
	D3DXVECTOR3 vPosInStartClient;					// 実行時のクライアント領域での座標
	D3DXVECTOR3 vSclInStartClient;					// 実行時のクライアント領域でのサイズ
	D3DXCOLOR	col;								// 色
	int			nCnt;
	bool		bUse;								// 使用してるかどうか
	bool		bChoice;							// 選択フラグ
